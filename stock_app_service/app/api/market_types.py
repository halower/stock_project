    ...
    try:
        ...
        for ts_code, stock_data_str in stock_list_data.items():
            try:
                ...
                if category not in market_stats:
                ...
                market_stats[category]["count"] += 1
                
                # 只保留前10个代码作为示例
                if len(market_stats[category]["codes"]) < 10:
                    market_stats[category]["codes"].append({
                        "code": stock_data.get('symbol', ts_code.split('.')[0]),
        ...
        # 转换为列表并排序
        stats_list = list(market_stats.values())
        stats_list.sort(key=lambda x: x["count"], reverse=True)
        
        total_count = sum(item["count"] for item in stats_list)
        
        logger.info(f"市场类型统计: {len(stats_list)} 个类型, 共 {total_count} 只股票/ETF")
        ...
        logger.error(f"获取市场类型统计失败: {str(e)}")
        import traceback
        logger.error(f"详细错误: {traceback.format_exc()}")
        
        return {
            "code": 500,
            "message": f"获取市场类型统计失败: {str(e)}",
            "data": {
                "stats": [],
                "total": 0
            }
