...
class ETFDataProvider(str, Enum):
    """ETF数据提供商枚举"""
    EASTMONEY = "eastmoney"  # 东方财富（唯一数据源）


class ETFRealtimeServiceV2:
    """
    ETF实时行情服务V2
    ...
    def __init__(
        ...
        logger.info(f"ETF实时服务V2初始化: provider={default_provider}, proxy={'enabled' if proxy_manager and proxy_manager.enable_proxy else 'disabled'}")
    ...
    def get_all_etfs_realtime(self, provider: Optional[str] = None) -> Dict[str, Any]:
        ...
        # 只使用东方财富数据源
        providers_to_try = [ETFDataProvider.EASTMONEY]
            ...
            for retry in range(self.retry_times):
                ...
                try:
                    ...
                    if prov == ETFDataProvider.EASTMONEY:
                        result = self._fetch_eastmoney_etf(proxy_info)
                    elif prov == ETFDataProvider.SINA:
                        result = self._fetch_sina_etf(proxy_info)
                    else:
                        continue
    ...
    def _fetch_single_etf_page_thread(
        self,
        url: str,
        page_num: int,
        base_time: int,
        ...
        try:
            params = {
                ...
                "invt": "2",
                "fid": "f3",
                "fs": "b:MK0021,b:MK0022,b:MK0023,b:MK0024",  # ETF市场
    ...
    def _fetch_all_etf_pages_concurrent(
        self,
        url: str,
        total_pages: int,
        proxies: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """使用多线程并发获取所有ETF页面（使用同一个代理）"""
        base_time = int(time.time() * 1000)
        all_data = []
        failed_pages = []
        
        logger.info(f"开始多线程并发获取{total_pages}个ETF页面...")
        
        # 使用线程池并发获取
        with ThreadPoolExecutor(max_workers=10) as executor:
            # 提交所有任务
            future_to_page = {
                executor.submit(self._fetch_single_etf_page_thread, url, page, base_time, proxies): page
        ...
        if failed_pages:
            error_msg = f'部分页面失败: {failed_pages[:3]}'
        ...
        logger.info(f"多线程并发获取完成，共{len(all_data)}条ETF数据")
        return {
            'success': True,
            'data': all_data,
            'count': len(all_data)
    ...
    def _fetch_eastmoney_etf(self, proxy_info: Optional[ProxyInfo] = None) -> Dict[str, Any]:
        """
        从东方财富获取ETF实时行情（内部多线程并发）
        ...
        try:
            ...
            logger.info(f"使用{'代理' if proxies else '直连'}获取东方财富ETF数据")
            ...
            logger.info(f"总共{total}条ETF数据，需要多线程并发获取{total_pages}页")
            
            # 使用多线程并发获取（使用同一个代理）
            result = self._fetch_all_etf_pages_concurrent(url, total_pages, proxies)
            ...
            for item in all_data:
                try:
                    ...
                    logger.debug(f"解析东方财富ETF数据行失败: {e}")
                    continue
...
# 全局单例
_etf_realtime_service_v2: Optional[ETFRealtimeServiceV2] = None


def get_etf_realtime_service_v2(
    proxy_manager: Optional[ProxyManager] = None,
    **kwargs
    ...
    Args:
        ...
        **kwargs: 其他配置参数
    """
    global _etf_realtime_service_v2
    
    if _etf_realtime_service_v2 is None:
        import os
        
        default_provider = os.getenv('ETF_REALTIME_PROVIDER', kwargs.get('default_provider', 'eastmoney'))
        auto_switch = os.getenv('ETF_AUTO_SWITCH', 'true').lower() in ('true', '1', 'yes')
        retry_times = int(os.getenv('ETF_RETRY_TIMES', kwargs.get('retry_times', 3)))
        timeout = int(os.getenv('ETF_TIMEOUT', kwargs.get('timeout', 10)))
        
        _etf_realtime_service_v2 = ETFRealtimeServiceV2(
    ...
    return _etf_realtime_service_v2
