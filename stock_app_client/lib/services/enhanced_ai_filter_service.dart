...
- **止损价设置原则（必须严格遵守）**：
  ...
  * 优先选择：当前价下方3-5%的价位
  * 如果第1支撑位距离当前价超过8%，则**不使用**该支撑位，改用当前价下方5-8%
  * 例如：当前价100元，止损价应在92-97元之间
  
- **目标价设置原则**：
  * 优先选择第1阻力位（如果距离合理，通常5-15%）
  * 如果第1阻力位太近（<3%），选择第2阻力位
  * 如果第1阻力位太远（>20%），选择当前价上方10-15%
  * 必须确保盈利空间至少是止损空间的2倍
...
- **专业示例（正确）**：
  * 当前价：¥100.00
  * 第1支撑：¥95.00（-5.0%）✅ 距离合理
  * 第1阻力：¥110.00（+10.0%）✅ 距离合理
  * 止损价：¥95.00（风险-5.0%）✅ 控制在3-8%范围内
  * 目标价：¥110.00（盈利+10.0%）✅ 
  * 盈亏比：10.0% / 5.0% = 2.0:1 ✅ 符合要求

- **错误示例（必须避免）**：
  * 当前价：¥100.00
  * 第1支撑：¥70.00（-30%）❌ 距离太远，不可使用！
  * 止损价应调整为：¥95.00（-5%）或¥92.00（-8%）
  * **绝对不能使用距离当前价超过10%的支撑位作为止损！**
